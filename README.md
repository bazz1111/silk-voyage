...
把本仓库放到该 agent 的 skills 目录，保证 `SKILL.md` 在目录根下。路径因 agent 而异，例如：

```text
~/.cursor/skills/silk-voyage/SKILL.md
```

Windows 示例：`%USERPROFILE%\.cursor\skills\silk-voyage\SKILL.md`

装好后按该 agent 的惯例重新加载 skills（例如新开一轮对话）。

## 用法

丢一张旅行/风景照片，说「绢上旅行」「上图下卷」或点名 `silk-voyage`。必须有照片；一图一海报。
